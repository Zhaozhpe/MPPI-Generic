...
struct DubinsCostMapParams : public CostParams<DYN_T::CONTROL_DIM>
{
  ...
  // Coefficients for quadratic state error.
  float s_coeffs[DYN_T::OUTPUT_DIM] = { 0 };

  int current_time = 0;
  int timesteps_total = 100;

  // Weight for the cost-map term.
  float map_cost_weight = 20.0f;
  ...
  // // Transformation parameters for mapping world coordinates to normalized texture coordinates.
  // float3 r_c1;  // first column of transform matrix
  // float3 r_c2;  // second column of transform matrix
...
protected:
  ...
  __host__ __device__ float queryTextureTransformed(float x, float y) const
  {
    float u, v;
    coorTransform(x, y, &u, &v);
#ifdef __CUDA_ARCH__
    // printf("queryTextureTransformed (device): x=%f, y=%f, u=%f, v=%f\n", x, y, u, v);
    float tex_val = tex2D<float4>(costmap_tex_d_, u, v).x;
